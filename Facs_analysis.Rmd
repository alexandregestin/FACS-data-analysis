...
tsne_expbe <- readRDS(object_name)
...
sample_per_cluster(pheno_expbe_al[[2]]$membership) # what does this do?
# returns error : Error in order(y) : unimplemented type 'list' in 'orderVector1'
# this worked the first time I ran it and then it didn't - is there some variable that needs to be reset?
...
k = '250'
settings = "biex transform, aligned"
plot_title = paste("Phenograph clusters, k =",k,settings,sep = " ") 

# again this worked the first time through and now it does not - probably because the sample_per_cluster was not run but I'm not sure

pCtsne <- plot_tsne_clust(tsne_expbe, pheno_expbe_al[[2]]$membership,plot_title)
pCtsne

plot_tsne_clust(tsne_expbe, pheno_expbe_al[[2]]$membership,"Phenograph clustering (k=250) of biexp transformed aligned scaled values of 9 MBOs with 1578 cells each")
#dev.off()
heatmap_cluster(rename_markers(expbe_al),pheno_expbe_al[[2]]$membership,c(1,2,9))
# what does this vector c(1,2,9) select?
# works
# remove 'cell' as upper x axis label

hmap1 <- heatmap_cluster(rename_markers(expbe_al),pheno_expbe_al[[2]]$membership,c(1,2,9))

png(file= (paste(save_path,"heatmap_Rpheno_k250.png",sep="")),width=1000*1.618,height=1000)
hmap1
dev.off()
...
reference_table = "/home/rhalena/Documents/Documents/MyPapers/FACS/Analysis/ManualAnalysis/ExpessionCellPhenotypeTable3.csv"
norm_cellpheno=read.csv(file= reference_table)
